...
class LeadLockingServiceTest {
  ...
  @Autowired private LeadRepositoryJpa leadRepository;
  ...
  void shouldPreventLostUpdate_whenPessimisticLockUsed() throws Exception {
    ...
    Future<LeadStatusJpa> task1 =
        executor.submit(
            () -> {
              startLatch.await();
              Lead updated =
                  leadLockingService.convertLeadToDealWithLock(leadId, LeadStatusJpa.CONTACTED);
              doneLatch.countDown();
              return updated.getStatus();
            });

    Future<LeadStatusJpa> task2 =
        executor.submit(
            () -> {
              startLatch.await();
              Lead updated =
                  leadLockingService.convertLeadToDealWithLock(leadId, LeadStatusJpa.QUALIFIED);
              doneLatch.countDown();
              return updated.getStatus();
            });
  ...
  void shouldThrowOptimisticLockException_whenConcurrentUpdateWithoutLock() throws Exception {
    Lead lead =
        Lead.builder().phone("+7911").email("iv@gmail.com").status(LeadStatusJpa.NEW).build();
  ...
  void shouldDetectDeadlock_whenLeadsLockedInDifferentOrder() throws Exception {
    Lead leadFirst =
        Lead.builder().email("ivental@gmail.com").phone("+7911").status(LeadStatusJpa.NEW).build();
    Lead leadSecond =
        Lead.builder().email("ive@gmail.com").phone("+7912").status(LeadStatusJpa.NEW).build();
