...
class LeadControllerTest {
  ...
  void setUp() {
    ...
    existingLead = new Lead(validId, "test@example.com", "+79991234567", "Test", LeadStatus.NEW);
  }
  ...
  void shouldCreateLeadAndRedirectToList() throws Exception {
    Lead newLead =
        new Lead(
            UUID.randomUUID(), "new@example.com", "+79991234567", "New Company", LeadStatus.NEW);
    when(leadService.addLead("new@example.com", "+79991234567", "New Company", LeadStatus.NEW))
        .thenReturn(newLead);
    mockMvc
        .perform(
            post("/leads")
        ...
        .andExpect(status().is3xxRedirection())
        .andExpect(redirectedUrl("/leads"));
    verify(leadService).addLead("new@example.com", "+79991234567", "New Company", LeadStatus.NEW);
  }
  ...
  void shouldFilterBySearchTerm() throws Exception {
    ...
    Lead expectedLead =
        new Lead(
            UUID.randomUUID(), "test@example.com", "+79991234567", "Test Company", LeadStatus.NEW);
  ...
  void shouldCombineSearchAndStatusFilters() throws Exception {
    ...
    LeadStatus status = LeadStatus.NEW;
    Lead expectedLead =
        new Lead(UUID.randomUUID(), "contact@acme.com", "+79990000000", "Acme Inc", status);
